# Serveur Web - VLAN 120 (Salmon)

## Présentation

Ce serveur Web est hébergé sur le **VLAN 120 (Salmon)** et met à disposition un site Internet accessible depuis le réseau interne ainsi que depuis le réseau extérieur grâce aux règles de traduction d'adresses (NAT/PAT) configurées sur le routeur **R-9**.
...
| Paramètre | Valeur |
|-----------|---------|
| Nom | Serveur Web VLAN 120 |
| VLAN | 120 (Salmon) |
| Adresse IP | 192.168.12.250 |
| Masque | 255.255.255.0 |
| Passerelle | 192.168.12.254 (HSRP) |
| Service | HTTP |
...
|------------------|----------------|------|
| 100.0.18.253 | 192.168.12.250 | TCP/8808 |

Configuration Cisco utilisée :

```cisco
ip nat inside source static tcp 192.168.12.250 80 100.0.18.253 8808
```
...
- Les postes du VLAN 120 accèdent au serveur.
