# Serveur Web - VLAN 70 (Gold)

## Présentation

Ce serveur Web est hébergé sur le **VLAN 70 (Gold)** et met à disposition un site Internet accessible depuis le réseau interne ainsi que depuis le réseau extérieur grâce aux règles de traduction d'adresses (NAT/PAT) configurées sur le routeur **R-9**.
...
| Paramètre | Valeur |
|-----------|---------|
| Nom | Serveur Web VLAN 70 |
| VLAN | 70 (Gold) |
| Adresse IP | 192.168.7.250 |
| Masque | 255.255.255.0 |
| Passerelle | 192.168.7.254 (HSRP) |
| Service | HTTP |
...
|------------------|----------------|------|
| 100.0.18.253 | 192.168.7.250 | TCP/8000 |

Configuration Cisco utilisée :

```cisco
ip nat inside source static tcp 192.168.7.250 80 100.0.18.253 8000
```
...
- Les postes du VLAN 70 accèdent au serveur.
